...
class HomeViewController: UIViewController, RPPreviewViewControllerDelegate {
	...
	@objc func startRecording() {
        ...
        videoOutputURL = URL(fileURLWithPath: documentsPath.appendingPathComponent("MyVideo.mp4"))
		...
		RPScreenRecorder.shared().startCapture(handler: {(sample, bufferType, error) in
			...
			if CMSampleBufferDataIsReady(sample){
				...
				if self.videoWriter.status == AVAssetWriter.Status.failed {
				   print("Error occured, status = \(self.videoWriter.status.rawValue), \(self.videoWriter.error!.localizedDescription) \(String(describing: self.videoWriter.error))")
				   return
			   }
